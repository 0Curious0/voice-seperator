...
def update_W(W, x, learning_rate):
    ...
    updated_W = W + learning_rate * (-np.outer(np.sign(W @ x.reshape(-1, 1)), x) + np.linalg.inv(W.T))
...
def load_data():
    mix = np.loadtxt(os.path.join(BASE_DIR,'../data/mix.dat'))
    return mix

def save_W(W):
    np.savetxt(os.path.join(BASE_DIR,'../output/W.txt'),W)

def save_sound(audio, name):
    scipy.io.wavfile.write(os.path.join(BASE_DIR,'../output/{}.wav'.format(name)), Fs, audio)
